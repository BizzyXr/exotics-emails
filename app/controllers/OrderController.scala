...
//         to = Seq("sales@qualitywaxandcarts.com","champiponsi@gmail.com"),
         to = Seq("sales@coloradoexoticshop.com"),
  ...
  def contact = Action.async(parse.json) { implicit request =>
    request.body.validate[Contact].map { contact =>
      mailerClient.send(Email(
        subject = "New Message",
        from = "noreply@brimweb.com",
//        to = Seq("info@qualitywaxandcarts.com", "champiponsi@gmail.com"),
        to = Seq("info@coloradoexoticshop.com"),
